...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ URL::to('laporan/siswa-masuk') }}"><i class="active"></i> Pendaftaran</a></li>
    </ol>
</section>
        ...
        <div class="col-lg-12">
            <div class="box">
                <div class="box-header">
                    <h2 class="box-title">Pendaftaran {{ $classification->name }}</h2>
                </div>
                        ...
                        <small>
                            ...
                            <tbody>
                                @foreach($registrations as $registration)
                                    <tr>
                                        <td>{{ $registration->issue->issue }} / {{ $registration->issue->student->name }}</td>
                                        <td>{{ date('d-m-Y', strtotime($registration->issue->student->birthdate)) }}</td>
                                        <td>{{ $registration->issue->student->father_name }}</td>
                                        <td>{{ $registration->issue->student->mother_name }}</td>
                                        <td>{{ $registration->issue->student->address }}</td>
                                        <td>{{ $registration->issue->student->contact }}</td>
                                        <td>{{ $registration->issue->student->email }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </small>
...
<script type="text/javascript">
    ...
    function filter()
    {
        ...
        window.location = "{{ URL::to('laporan/siswa-masuk/filter') }}/"+classification_id;
    }
</script>
@stop