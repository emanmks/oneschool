...
<section class="content-header">
    <h1>Laporan Menunggak</h1>
    <ol class="breadcrumb">
        <li><a href="{{ URL::to('/') }}"><i class="fa fa-desktop"></i> Home</a></li>
        <li><a href="{{ URL::to('laporan') }}">Laporan</a></li>
        <li><a href="{{ URL::to('laporan/menunggak') }}"><i class="active"></i> Menunggak</a></li>
    </ol>
</section>
                    ...
                    <table class="table table-condensed" id="data-table">
                        ...
                        <tbody>
                            ...
                            @foreach($installments as $installment)
                                <tr>
                                    <td>{{ $installment->receivable->issue->issue }} / {{ $installment->receivable->issue->student->name }}</td>
                                    <td>{{ $installment->receivable->issue->student->father_name }}</td>
                                    <td>{{ $installment->receivable->issue->student->mother_name }}</td>
                                    <td>{{ $installment->receivable->issue->student->address }}, {{ $installment->receivable->issue->student->contact }}</td>
                                    <td>Rp{{ number_format($installment->balance,2,',',',') }}</td>
                                </tr>
                                <?php $total += $installment->balance; ?>
                            @endforeach
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    function filter()
    {
        ...
        window.location = "{{ URL::to('laporan/menunggak/filter') }}/"+curr_month+"/"+curr_year;
    }
</script>
@stop