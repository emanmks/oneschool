...
<section class="content-header">
    <h1>Deduction Payroll</h1>
    <ol class="breadcrumb">
        <li><a href="{{ URL::to('/') }}"><i class="fa fa-desktop"></i> Home</a></li>
        <li><a href="#"> Kepegawaian</a></li>
        <li><a href="{{ URL::to('deduction') }}"><i class="active"></i> Deduction Payroll</a></li>
    </ol>
</section>
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <div class="box-title">
                        <button class="btn btn-primary" onclick="update({{ $deduction->id }})"><i class="fa fa-floppy-o"></i> Update</button>
                    </div>
                </div>
                ...
                <div class="box-body">
                    <form class="form-horizontal" role="form">
                    	<div class="form-group">
                            ...
                            <div class="col-lg-5">
                                <input type="text" name="employees" class="form-control" data-provide="typeahead" value="ID: {{ $deduction->employee->employee_id }} Kode: {{ $deduction->employee->code }} Name: {{ $deduction->employee->name }}">
                                <input type="hidden" name="employee" value="{{ $deduction->employee_id }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label" for="classifications">Jenis Deduction</label>
                            <div class="col-lg-5">
                                ...
                                <script type="text/javascript">
                                	$('[name=classifications]').val("{{ $deduction->classification_id }}");
                                </script>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-lg-3">
                                <input name="nominal" type="text" class="form-control currency" value="{{ number_format($deduction->nominal,2,',',',') }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label" for="release_date">Tanggal Terbit</label>
                            <div class="col-lg-2">
                                <input name="release_date" type="text" class="form-control" value="{{ date('Y-m-d', strtotime($deduction->release_date)) }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label" for="comments">Tambahan Informasi</label>
                            <div class="col-lg-8">
                                <textarea name="comments" class="textarea form-control" placeholder="Tambahkan Informasi dan Penjelasan disini"style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ $deduction->comments }}</textarea>
                            </div>
                        </div>
        ...
        if(classification_id != '0' && employee_id != '0')
        {
            $.ajax({
                ...
                data:{
                	...
                	comments : comments},
                success:function(){
                    window.location = "{{ URL::to('deduction') }}/"+id;
                }
            });
        }
        else
